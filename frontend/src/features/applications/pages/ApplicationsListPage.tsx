...
function ApplicationsListPage() {
 ...
 const visibleApps = useMemo(() => buildVisibleApps(apps, { showAll, searchTerm: debouncedSearchTerm }),
 [apps, showAll, debouncedSearchTerm],
 )

 const { pageItems: desktopPageItems, meta: desktopMeta } = useMemo(() => paginate(visibleApps, { pageSize, currentPage }),
 ...
 const pageNumbersToShow = useMemo(() => getPageNumbersToDisplay(currentPageSafe, totalPages),
 [currentPageSafe, totalPages],
 )

 if (loading) return <div className="text-on-surface">{t('applications.loading')}</div>
 if (error)
 return (<div role="alert" aria-live="assertive" className="text-danger dark:text-danger">
 {error}
 </div>
 )
 if (!apps || apps.length === 0) return <p className="text-on-surface">{t('applications.noApplications')}</p>
 ...
 actions={
 !isMobile ? (<ApplicationsToggleButton showAll={showAll} onToggle={handleToggleShowAll} />
 ) : null
 }
 />

 <div className="w-full mb-6 sm:mb-8 rounded-2xl border border-primary/10 bg-surface-container-low/80 shadow-[0_2px_12px_-4px_rgba(113,75,103,0.12),0_0_0_1px_rgba(148,163,184,0.08)] dark:shadow-none p-4 sm:p-5">
 <div className="flex flex-row flex-wrap items-center gap-2 sm:gap-5">
 <div className="relative flex-1 min-w-0 sm:max-w-[480px]">
 <input
 type="search"
 className="w-full py-2.5 px-4 rounded-full border border-primary/20 bg-primary/5 text-sm text-on-surface outline-none shadow-[0_4px_10px_-6px_rgba(113,75,103,0.4),0_0_0_1px_rgba(148,163,184,0.3)] dark:shadow-none placeholder:text-on-surface-variant focus:border-warning-dark focus:bg-warning-light/30 focus:shadow-[0_6px_14px_-8px_rgba(245,158,11,0.6),0_0_0_1px_rgba(245,158,11,0.5)] dark:focus:shadow-none"
 placeholder={
 ...
 <label className="flex items-center gap-2.5 text-sm text-on-surface-variant shrink-0">
 <span className="whitespace-nowrap">{t('applications.itemsPerPage')}</span>
 <select
 value={pageSize}
 onChange={(e) => handlePageSizeChange(Number(e.target.value))}
 className="py-2.5 pl-3.5 pr-9 rounded-full border border-primary/20 bg-primary/5 text-on-surface text-sm font-medium outline-none appearance-none cursor-pointer shadow-[0_2px_8px_-4px_rgba(113,75,103,0.2),0_0_0_1px_rgba(148,163,184,0.2)] dark:shadow-none focus:border-warning-dark focus:bg-warning-light/30 focus:shadow-[0_4px_12px_-6px_rgba(245,158,11,0.35),0_0_0_1px_rgba(245,158,11,0.4)] dark:focus:shadow-none bg-[length:1.25rem] bg-[right_0.5rem_center] bg-no-repeat"
 ...
 {totalItems > 0 && (<div className="w-full mt-6 sm:mt-8 pt-5 sm:pt-6 border-t border-outline-variant flex flex-col items-center gap-3">
 {!isMobile && totalItems > pageSize && (<div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3">
 ...
 item ==='ellipsis' ? (<span key={`ellipsis-${idx}`} className="px-1.5 text-on-surface-variant text-sm" aria-hidden="true">…</span>
 ) : (<Button
 key={item}
 ...
 {isMobile && canLoadMoreMobile && (<Button variant="primary" size="sm" onClick={handleLoadMore} aria-controls="applications-results">
 {t('applications.loadMore')}
 </Button>
 )}

 {isMobile && (<span className="sr-only" aria-live="polite">
 {t('applications.showing', { start: 1, end: mobileEndIndex, total: totalItems, current: currentPageSafe, totalPages })}
 </span>
 )}

 <span className="hidden sm:block text-xs sm:text-sm text-on-surface-variant text-center w-full">
