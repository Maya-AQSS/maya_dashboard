...
function ToolsListPage() {
 ...
 const visibleTools = useMemo(() => buildVisibleTools(tools, { showAll, searchTerm: debouncedSearchTerm }),
 [tools, showAll, debouncedSearchTerm],
 )

 const { pageItems: desktopPageItems, meta: desktopMeta } = useMemo(() => paginate(visibleTools, { pageSize, currentPage }),
 ...
 if (loading) return <div className="text-on-surface">{t('tools.loading')}</div>
 if (error)
 return (<div role="alert" aria-live="assertive" className="text-danger dark:text-danger">
 {error}
 </div>
 )
 if (!tools || tools.length === 0) return <p className="text-on-surface">{t('tools.noTools')}</p>
 ...
 >
 {PAGE_SIZE_OPTIONS.map((n) => (<option key={n} value={n}>
 {n}
 </option>
 ))}
 </select>
 ...
 item ==='ellipsis' ? (<span
 key={`ellipsis-${idx}`}
 className="px-1.5 text-on-surface-variant text-sm"
 aria-hidden="true"
 >
 …
 </span>
 ...
 {isMobile && canLoadMoreMobile && (<Button variant="primary" size="sm" onClick={handleLoadMore} aria-controls="tools-results">
 {t('tools.loadMore')}
 </Button>
 )}
