...
function ApplicationsListPage() {
  ...
  if (loading) return <div className="text-gray-900 dark:text-odoo-dark-text">{t('applications.loading')}</div>
        ...
        <div className="w-full mt-6 sm:mt-8 pt-5 sm:pt-6 border-t border-gray-100 dark:border-odoo-dark-border flex flex-col items-center gap-3">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3">
              ...
              <nav className="flex items-center gap-1" aria-label={t('applications.paginationLabel')}>
                {pageNumbersToShow.map((item, idx) =>
                  item === 'ellipsis' ? (
                    <span key={`ellipsis-${idx}`} className="px-1.5 text-gray-500 dark:text-odoo-dark-muted text-sm" aria-hidden="true">…</span>
                  ) : (
                    <button
                      key={item}
                      type="button"
          ...
          {isMobile && canLoadMoreMobile && (
            <button
              ...
              aria-controls="applications-results"
            >
