#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# up.sh — Script de arranque de Maya Dashboard
#
# Uso:
...
upsert_env_var frontend/.env VITE_API_URL             "${VITE_API_URL:-http://maya_dashboard_api.localhost/api/v1}"
...
if [[ "$NEED_KEY_GENERATE" == true ]]; then
    ...
    for i in $(seq 1 60); do
      if docker exec maya_dashboard_backend test -f /var/www/html/vendor/autoload.php 2>/dev/null; then
        NEW_KEY=$(docker exec maya_dashboard_backend php artisan key:generate --show 2>/dev/null || true)
...
echo -e "  ${GREEN}Frontend:${NC}         http://maya_dashboard.localhost"
echo -e "  ${GREEN}Backend API:${NC}      http://maya_dashboard_api.localhost/api/v1"
...
echo -e "    Backend:   http://localhost:${BACKEND_PORT:-8003}"
